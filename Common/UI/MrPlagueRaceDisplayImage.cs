...
{
    public class MrPlagueRaceDisplayImage : UIElement
    {
        private Texture2D _texture;
		private Texture2D UI_RaceEnvironmentImageIcon()
		{
			return GetTexture(RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].RaceEnvironmentIcon);
		}
		private Texture2D UI_RaceEnvironmentOverlay1ImageIcon()
		{
			return GetTexture(RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].RaceEnvironmentOverlay1Icon);
		}
		private Texture2D UI_RaceEnvironmentOverlay2ImageIcon()
		{
			return GetTexture(RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].RaceEnvironmentOverlay2Icon);
		}
		public Texture2D UI_RaceDisplayMaleImageIcon()
		{
			return GetTexture(RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].RaceDisplayMaleIcon);
		}
		public Texture2D UI_RaceDisplayFemaleImageIcon()
		{
			return GetTexture(RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].RaceDisplayFemaleIcon);
		}

		public MrPlagueRaceDisplayImage(Texture2D texture)
        {
            if (!Main.dedServ)
            {
                _texture = texture;
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
			...
			if (RaceLoader.Races[MrPlagueRaceSelection.RaceIndex].DarkenEnvironment)
			{
